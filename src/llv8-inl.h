...
ACCESSOR(SharedFunctionInfo, function_data, shared_info()->kFunctionDataOffset, Value)
...
ACCESSOR(SharedFunctionInfo, script_or_debug_info, shared_info()->kScriptOrDebugInfoOffset, HeapObject)
...
ACCESSOR(UncompiledData, inferred_name,
         uncompiled_data()->kInferredNameOffset, Value)
ACCESSOR(UncompiledData, start_position,
         uncompiled_data()->kStartPositionOffset, int32_t)
ACCESSOR(UncompiledData, end_position,
         uncompiled_data()->kEndPositionOffset, int32_t)

Value SharedFunctionInfo::GetInferredName(Error& err) {
  if (v8()->uncompiled_data()->kInferredNameOffset == -1) return inferred_name(err);
  ...
  if (!err.Fail()) {
    ...
    if (!(err.Fail() && String::IsString(v8(), maybe_inferred_name, err))) return maybe_inferred_name;
  }
